"""
VEV_4000-only trader, R4 v2.

Diff from v1:
  - Layer 2 (VE hedge) REMOVED. We accept naked delta exposure on VEV_4000
    inventory. The inventory-skew on quote prices and the natural mean
    reversion of VEV_4000 (= VE - 4000) handle position management.
  - QUOTE_SIZE bumped 30 -> 60. We quote bigger to capture more of Mark 38's
    flow when our 1-tick undercut wins price priority.
  - VE book is still read (needed for Layer 3 fair-value computation), but
    we never place orders on VE.

Architectural notes carried over from v1:
  - State persisted via JSON traderData (Lambda-safe).
  - Each tick rebuilds all desired orders from scratch.
  - Position limits checked before emitting any order.
  - All defensive empty-book handling.
"""
...
class Trader:
    ...
    QUOTE_SIZE = 60           # bumped from 30; we want more of Mark 38's flow
