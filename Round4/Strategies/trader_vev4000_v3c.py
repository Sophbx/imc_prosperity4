"""
VEV_4000-only trader, R4 v3c.

Diff from v3a: INV_TILT 0.01 -> 0 (no skew at all).

The most aggressive version. We don't tilt our quotes based on inventory
at all. Position-limit headroom (300/-300) is the only natural brake.

Hypothesis: v3a's day-1 inventory drifted to +45 and the strategy gained
$873 vs v2 from VEV_4000's natural mean reversion back. With TILT=0,
inventory could drift to +100 / +150 on similar days, multiplying the
mean-reversion harvest.

Risk: if VEV trends one direction without reverting, we accumulate huge
loss. No internal stopper — must rely on hard 300 limit.
"""
...
class Trader:
    ...
    INV_TILT = 0.0            # NO inventory skew at all — let pos drift to limits
