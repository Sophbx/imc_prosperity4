"""
VE-only trader, R4 v7.

Diff from v5: COMBINES Mark 55 anti-signal (primary) with Mark 67/49
bullish signal (secondary boost).

  - Mark 55 SELLS  → signal +    (anti-signal: he sold low)
  - Mark 55 BUYS   → signal -    (anti-signal: he bought high)
  - Mark 67 BUYS   → signal +    (extra boost when he prints)
  - Mark 49 SELLS  → signal +    (extra boost when he prints)

The Mark 67/49 contribution is given a smaller per-lot weight than Mark
55 because (a) they fire less often (~90/day vs ~400/day), and (b) the
v4 lesson: if these one-sided signals dominate, position pins at +200.

The combined signal is clamped to ±MAX_FAIR_SHIFT. Mark 55 buying
provides the natural "down" bias to balance Mark 67/49's permanent "up".
If Mark 55 isn't buying enough, we still cap at MAX_FAIR_SHIFT and don't
runaway.
...
class Trader:
    ...
    TRIGGER_MIN_QTY = 3
    M55_SIGNAL_PER_LOT = 0.04     # Mark 55 anti (primary, two-sided)
    M67_M49_SIGNAL_PER_LOT = 0.04 # Mark 67 buy / Mark 49 sell (secondary, +)
    SIGNAL_DECAY = 0.9995
    MAX_FAIR_SHIFT = 2.0
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        # ----- Combined signal: Mark 55 anti (primary) + Mark 67/49 (boost) -----
        signal *= self.SIGNAL_DECAY
        for trade in state.market_trades.get(self.VE_SYM, []) or []:
            qty = int(trade.quantity)
            if qty < self.TRIGGER_MIN_QTY:
                continue
            # Primary: Mark 55 anti (two-sided)
            if trade.seller == "Mark 55":
                signal += qty * self.M55_SIGNAL_PER_LOT
            elif trade.buyer == "Mark 55":
                signal -= qty * self.M55_SIGNAL_PER_LOT
            # Secondary: Mark 67 buys / Mark 49 sells (one-sided +)
            if trade.buyer == "Mark 67":
                signal += qty * self.M67_M49_SIGNAL_PER_LOT
