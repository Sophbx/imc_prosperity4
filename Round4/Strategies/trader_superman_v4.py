"""
trader_superman_v4.py — v1 + Mark 22 limit detection on VEV_5300 only.

Diff from v1:
  - Track Mark 22 cumulative short on VEV_5300 (built from market_trades).
  - When his short ≤ -180, become aggressive: cross spread to BUY 30 lots
    (in addition to normal swing accumulation). When he's near his limit
    (~-250), his selling pressure decays → strike rallies → we want long.

Why ONLY VEV_5300:
  - We tested v2 with M22 detection on 5100/5200/5300/5400.
  - 5100/5200/5400: M22 never approaches limit; cross-buy just bleeds spread.
  - 5300: ONLY strike where M22 reliably hits -250 (Day 3 EOD).
  - Surgical approach: target the one strike where the alpha is real.

v2 also tried M67 acceleration delay (hurt) and size tuning (hurt).
v4 drops both — minimum-change improvement.
...
class Trader:
    ...
    # ---- NEW v4: Mark 22 limit detection on VEV_5300 ----
    M22_LIMIT_LEVEL = -180
    M22_CROSS_QTY = 30
    M22_TRACKED_STRIKE = "VEV_5300"
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        # 4. Update Mark 22 cumulative short on VEV_5300
        m22_pos = mem.get("m22_5300", 0)
        for t in state.market_trades.get(self.M22_TRACKED_STRIKE, []) or []:
            if t.buyer == "Mark 22":
                m22_pos += int(t.quantity)
            elif t.seller == "Mark 22":
                m22_pos -= int(t.quantity)
        mem["m22_5300"] = m22_pos

        # 4b. VEV swing strikes (5100, 5200, 5300, 5400)
        for strike in self.SWING_VEV_STRIKES:
            sym = f"VEV_{strike}"
            # Pass m22 position only for tracked strike
            m22_pos_arg = m22_pos if sym == self.M22_TRACKED_STRIKE else 0
            orders = self._trade_vev_swing_with_m22(state, sym, m22_pos_arg)
            if orders:
                result[sym] = orders
    ...
    # ==================================================================
    # VEV swing (5100, 5200, 5300, 5400) — with optional Mark 22 detection
    # ==================================================================
    def _trade_vev_swing_with_m22(
        ...
        # Mark 22 limit detection (only when m22_short_pos != 0, i.e. tracked strike)
