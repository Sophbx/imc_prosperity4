"""
VEV_4000-only trader, R4 v3a.

Diff from v2: INV_TILT lowered 0.04 -> 0.01.

Hypothesis: at v2's INV_TILT=0.04 we never hit pos > 15 / 300 limit. The
inventory skew is binding too early — we leave room for mean-reversion
alpha on the table. By lowering tilt 4x we let inventory drift wider
(target range maybe ±60-100) and ride VEV_4000's natural mean reversion
back to 0 for extra PnL.

Risk: if VE/VEV trends instead of mean-reverting on the live day, larger
inventory means larger MtM swings.
"""
...
class Trader:
    ...
    INV_TILT = 0.01           # lowered from v2's 0.04 — let inventory drift
