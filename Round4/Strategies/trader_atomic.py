...
class Trader:
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        # 2. VE — swing + Mark 67/49 signal bias on quote prices
        ve_orders = self._trade_ve_swing(state, mem)
        if ve_orders:
    ...
    # VE — my swing v1
    # ==================================================================
    def _trade_ve_swing(self, state: TradingState, mem: Dict) -> List[Order]:
        ...
        # Direction A: Mark 67/49 signal bias on VE quote prices
        ve_swing_sig = float(mem.get("ve_swing_sig", 0.0)) * self.V4_SIGNAL_DECAY
        for t in state.market_trades.get(self.VE_SYM, []) or []:
            if t.buyer == "Mark 67":
                ve_swing_sig += self.V4_SIGNAL_PER_LOT * t.quantity
            elif t.seller == "Mark 49":
                ve_swing_sig += self.V4_SIGNAL_PER_LOT * t.quantity
        ve_swing_sig = max(-self.V4_SIGNAL_MAX, min(self.V4_SIGNAL_MAX, ve_swing_sig))
        mem["ve_swing_sig"] = ve_swing_sig
        sig_bias = int(round(ve_swing_sig))   # ticks of bias when bullish
        ...
        if ts < self.VE_ENTRY_START:
            ...
            if room > 0:
                ...
                # Bullish signal → bid higher (more aggressive accumulate)
                bid_px = bb + 1 + max(0, sig_bias)  # only positive bias on bid
                bid_px = min(bid_px, ba - 1)
                orders.append(Order(self.VE_SYM, bid_px, +size))
        elif ts < self.VE_EXIT_START:
            pass
        elif ts < self.VE_EXIT_FORCE:
            if pos > 0:
                size = min(self.VE_SWING_QUOTE_SIZE, pos)
                # No signal bias during exit — we want to sell aggressively
                ask_px = ba - 1
                orders.append(Order(self.VE_SYM, ask_px, -size))
        else:
            if pos > 0:
