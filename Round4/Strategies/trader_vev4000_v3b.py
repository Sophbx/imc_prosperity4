"""
VEV_4000-only trader, R4 v3b.

Diff from v2: quote 2 ticks inside Mark 14 instead of 1.

Hypothesis: at v2's 1-tick undercut we win price priority, but Mark 14
is adjacent at the L1 — there might still be FIFO ambiguity at adjacent
prices. Going 2 ticks inside is more decisive: we are unambiguously the
inside, Mark 14 is L2.

Cost: half-spread shrinks ~9.4 -> ~8.4 per share (-1 / share).
Benefit: presumably 100% of Mark 38's flow with no FIFO ambiguity.

Net is empirical — depends on whether the backtester was already giving
us ~100% of flow at 1-tick inside.
"""
