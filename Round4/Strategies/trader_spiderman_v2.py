"""
trader_spiderman_v1.py — Batman + Mark 22 sniper override on HG.

Combines the strengths of Batman (R3 v4 swing) and Superwoman (Mark 22 sniper):
  - Batman's swing state machine drives normal HG positioning (catches the
    big intraday reversals like Day 2's $32K capture)
  - When Mark 22 trades on HG, OVERRIDE the swing target to -SNIPE_QTY for
    SNIPE_HOLD ticks. After hold expires, swing logic resumes.
  - The override beats swing because Mark 22's HG trades predict large
    DOWNWARD moves (fwd_50k = -13 / -5 by side, big enough to overcome
    spread cost even when overriding a long swing position).

Backtest reference:
  Batman:     $125,855 (HG: $66,104)
  Superwoman:  $82,054 (HG: $22,303)
  Expected Spiderman: $130,000+ (HG ~$71K)
...
class Trader:
    ...
    # ------------------------------------------------------------------
    # HG — R3 v4 swing trader + Mark 22 sniper override
    # ------------------------------------------------------------------
    # Mark 22 sniper (override): when fires, force target = -SNIPE_QTY
    # v2: lighter sniper. Only triggers on Mark 22 BUY (stronger signal),
    # subtler bias (-30 not -100), shorter hold (15K not 50K)
    M22_SNIPE_BIAS = -30        # ADD this to swing target (not override)
    M22_SNIPE_HOLD = 15_000
    M22_TRIGGER_BUYER_ONLY = True   # only fire on Mark 22 BUY events

    # R3 v4 swing constants (state machine)
    HYDRO_FAIR = 10000.0
    HYDRO_SIGMA = 32.0
        ...
        for t in state.market_trades.get(self.HG_SYM, []) or []:
            ...
            if self.M22_TRIGGER_BUYER_ONLY:
                ...
                if t.buyer == "Mark 22" or t.seller == "Mark 22":
                    triggered = True
            if triggered:
                m22_hold_until = ts + self.M22_SNIPE_HOLD
        mem["m22_hg_hold_until"] = m22_hold_until

        swing_target = self._hydro_target(mid, ts, hd)

        # v2: ADD bias to swing target (don't override)
        if ts < m22_hold_until:
            target = swing_target + self.M22_SNIPE_BIAS
            target = max(-self.HG_LIMIT, min(self.HG_LIMIT, target))
        else:
            target = swing_target
