...
class Trader:
    ...
    def _trade_hg(self, state: TradingState, mem: Dict) -> List[Order]:
        ...
        # Mark 22 sniper state
        m22_hold_until = int(mem.get("m22_hg_hold_until", -1))
        hg_mt = state.market_trades.get(self.HG_SYM, []) or []
        if hg_mt:
            print(f"DBG ts={ts} HG mt count={len(hg_mt)} sample={[(t.buyer, t.seller, t.quantity) for t in hg_mt[:3]]}")
        for t in hg_mt:
            if t.buyer == "Mark 22" or t.seller == "Mark 22":
                m22_hold_until = ts + self.M22_SNIPE_HOLD
                print(f"M22_SNIPER_FIRE ts={ts} buyer={t.buyer} seller={t.seller} qty={t.quantity}")
        mem["m22_hg_hold_until"] = m22_hold_until

        # Compute swing target (state machine still updates internally)
        swing_target = self._hydro_target(mid, ts, hd)

        # Mark 22 sniper override: if active, force max-short
