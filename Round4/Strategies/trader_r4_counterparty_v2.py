...
Round 4 counterparty candidate v1:
  ...
  - v2 adds a small VEV_5300 Mark01-shadow experiment: long-only passive bids,
    capped tightly because day 3 is adverse.
...
class Trader:
    ...
    HYDRO_ALPHA = 0.03
    EXTRACT_ALPHA = 0.05
    VE_MARK67_DECAY_TS = 8_000.0
    VE_MARK67_BIAS_PER_LOT = 0.08
    VE_MARK67_MAX_BIAS = 2.0
    VEV5300_SHADOW_CAP = 80
    VEV5300_SHADOW_SIZE = 5
    ...
    def trade_vev5300_mark01_shadow(self, state: TradingState) -> List[Order]:
        product = "VEV_5300"
        depth = state.order_depths.get(product)
        if depth is None:
            return []
        ...
        # R4 tape: Mark 01 is a passive long in VEV_5300 against Mark 22 flow.
        # Keep this intentionally one-sided and capped; it is a small overlay,
        # not a broad voucher model.
        if ba - bb < 2:
            return []
        ...
        limit = self.POSITION_LIMITS[product]
        qty = min(
            self.VEV5300_SHADOW_SIZE,
            max(0, self.VEV5300_SHADOW_CAP - position),
            max(0, limit - position),
        )
        if qty <= 0:
            return []
    ...
    def run(self, state: TradingState):
        ...
        vev5300_orders = self.trade_vev5300_mark01_shadow(state)
        if vev5300_orders:
            result["VEV_5300"] = vev5300_orders
