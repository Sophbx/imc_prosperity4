...
class Trader:
    ...
    def _trade_vev_swing(self, state: TradingState, sym: str) -> List[Order]:
        ...
        if ts < entry_start:
            ...
            # PHOENIX: aggressive crossing in first half of entry window to fill faster
            room = max(0, self.VEV_LIMIT - pos)
            if room > 0:
                # First half of window: cross spread to ASK (pay half-spread, fill fast)
                first_half = entry_start + (entry_end - entry_start) // 2
                if ts < first_half and pos < self.VEV_LIMIT * 0.7:
                    avail = -int(depth.sell_orders[ba])
                    qty = min(avail, self.VEV_SWING_QUOTE_SIZE, room)
                    if qty > 0:
                        orders.append(Order(sym, ba, +qty))
                else:
