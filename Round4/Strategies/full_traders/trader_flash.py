"""
trader_flash.py — Batman + Direction C: HG swing reversal accelerated by Mark 38 cumulative.

Idea: R3 v4 swing waits for 3.35σ drop from peak before flipping short → long.
That's slow. Mark 38 is a noise trader whose trades are anti-predictive
(after he buys, mid drops). When Mark 38's CUMULATIVE net buy flow grows,
the imminent reversal is more certain → flip earlier with tighter threshold.

Symmetric for long mode: when Mark 38 cum_net is significantly negative
(he's been net selling), expect price drop → exit long earlier.

Implementation:
  - Track m38_hg_cum: cumulative Mark 38 net buy (decays slowly)
  - When m38_hg_cum > +30 AND swing in short mode: reversal_z 3.35 → 2.0
  - When m38_hg_cum < -30 AND swing in long mode: rebound_z 1.20 → 0.6

batman.py — same as Batman EXCEPT HG _hydro_target accepts m38_cum context.
...
class Trader:
    ...
    # Direction C: Mark 38 cumulative acceleration
    M38_CUM_DECAY = 0.999          # half-life ~700 ticks
    M38_CUM_THRESH = 80            # |cum| above this triggers acceleration (was 30)
    HYDRO_REVERSAL_Z_FAST = 2.0    # accelerated reversal threshold
    HYDRO_REBOUND_Z_FAST = 0.6     # accelerated rebound threshold
    ...
    def _hydro_target(self, mid: float, timestamp: int, hd: Dict, m38_cum: float = 0.0) -> int:
        ...
        # Direction C v2: REVERSED logic
        # Mark 38 SELLS → price rises (+0.55 fwd). cum_38 < 0 = he's been selling
        #   = price has been rising = bad for our short = reversal/flip imminent
        # Mark 38 BUYS → price drops. cum_38 > 0 = he's been buying = price dropping
        #   = good for our long = extend the long (rebound delay)
        if mode == "short" and m38_cum < -self.M38_CUM_THRESH:
            reversal_z_eff = self.HYDRO_REVERSAL_Z_FAST  # 2.0 instead of 3.35
        else:
            reversal_z_eff = self.HYDRO_REVERSAL_Z
        if mode == "long" and m38_cum > self.M38_CUM_THRESH:
            rebound_z_eff = self.HYDRO_REBOUND_Z_FAST    # 0.6 — exit faster when price dropping
        else:
            rebound_z_eff = self.HYDRO_REBOUND_Z

        reversal = reversal_z_eff * self.HYDRO_SIGMA
        rebound = rebound_z_eff * self.HYDRO_SIGMA
    ...
    def _trade_hg(self, state: TradingState, mem: Dict) -> List[Order]:
        ...
        # Direction C: track Mark 38 cumulative on HG (with EWMA decay)
        m38_cum = float(mem.get("m38_hg_cum", 0.0)) * self.M38_CUM_DECAY
        for t in state.market_trades.get(self.HG_SYM, []) or []:
            if t.buyer == "Mark 38":
                m38_cum += t.quantity
            elif t.seller == "Mark 38":
                m38_cum -= t.quantity
        mem["m38_hg_cum"] = m38_cum
        ...
        target = self._hydro_target(mid, int(state.timestamp), hd, m38_cum)
