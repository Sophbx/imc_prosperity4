"""
trader_chronos.py — HG = swing state machine + passive MM during flat mode.

Bold idea: R3 v4 swing is idle 80%+ of the time (mode=flat with no entry
trigger fired). During those idle ticks, passive MM at bb+1 / ba-1 captures
Mark 38's flow steadily. Two alpha sources stacked.

Decision tree:
  if swing in active mode (short or long): use swing target, cross spread
  elif swing is flat and just had cooldown end: use swing target (= 0)
  elif swing is flat with no triggers active: inject passive MM quotes

The MM quotes only fire when no swing trade is needed THIS tick — they
share the same _trade_hg call but appear after the swing's order check.
...
class Trader:
    ...
    HYDRO_MAX_TRADE = 40
    HYDRO_LONG_ENTRY_Z = -0.85
    HYDRO_LONG_SIZE = 200
    # Passive MM (idle layer)
    HG_MM_QUOTE_SIZE = 30
    HG_MM_INV_TILT = 0.04
    HG_MM_MIN_HALF_SPREAD = 8
    HYDRO_COOLDOWN = 1200
    HYDRO_RESET_Z = 0.35
    ...
    def _hydro_target(self, mid: float, timestamp: int, hd: Dict) -> int:
        ...
        if mode == "flat":
            ...
            if short_size > 0:
                ...
                return -short_size
            # Direction B: long-from-flat trigger (mirror of short-from-flat)
            if z <= self.HYDRO_LONG_ENTRY_Z:
                hd["mode"] = "long_from_flat"
                hd["extreme"] = mid
                return self.HYDRO_LONG_SIZE
            return 0

        if mode == "long_from_flat":
            low = min(float(extreme or mid), mid)
            hd["extreme"] = low
            # Exit when price rebounds enough from trough
            if mid >= low + rebound:
                hd["mode"] = "flat"
                hd["extreme"] = None
    ...
    def _trade_hg(self, state: TradingState, mem: Dict) -> List[Order]:
        ...
        # IDLE LAYER: when swing is in flat mode and no swing order this tick,
        # inject passive MM quotes to capture Mark 38's flow during idle stretches.
        if hd.get("mode") == "flat" and target == 0 and not orders:
            mm_skew = -self.HG_MM_INV_TILT * position
            mm_bid_px = bb + 1
            mm_ask_px = ba - 1
            mm_bid_px = int(round(mm_bid_px + mm_skew))
            mm_ask_px = int(round(mm_ask_px + mm_skew))
            mm_bid_px = min(mm_bid_px, ba - 1)
            mm_ask_px = max(mm_ask_px, bb + 1)
            if mm_bid_px >= mm_ask_px:
                mm_bid_px = mm_ask_px - 1
            mm_bid_size = max(0, min(self.HG_MM_QUOTE_SIZE, self.HG_LIMIT - position))
            mm_ask_size = max(0, min(self.HG_MM_QUOTE_SIZE, self.HG_LIMIT + position))
            if mm_bid_size > 0:
                orders.append(Order(self.HG_SYM, mm_bid_px, +mm_bid_size))
            if mm_ask_size > 0:
                orders.append(Order(self.HG_SYM, mm_ask_px, -mm_ask_size))
        return orders
