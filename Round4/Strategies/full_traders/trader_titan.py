"""
trader_titan.py — HG = continuous z-band mean reverter (no state machine).

BOLD departure from R3 v4 swing approach.

Idea: instead of state machine (flat → short → long after big reversal),
use a CONTINUOUS position-as-function-of-z mapping. Every tick, regardless
of state, target position is determined by current z = (mid - 10000) / 32.

  z >= +1.65 →  target -200 (deep short)
  z >= +1.00 →  target -100
  z >= +0.50 →  target  -50
  |z|  < 0.50→  target    0
  z <= -0.50 →  target  +50
  z <= -1.00 →  target +100
  z <= -1.65 →  target +200 (deep long)

Multi-band lets us capture small swings (every 1σ oscillation) AND big
swings (the ±1.65 deep zones). No cooldown, no state — constantly
responsive.

Trade off: more churn (cross spread on every band transition) vs more
swing captures. Let backtest decide.
...
class Trader:
    ...
    def _trade_hg(self, state: TradingState, mem: Dict) -> List[Order]:
        ...
        # NEW: continuous z-band target (no state machine)
        z = (mid - self.HYDRO_FAIR) / self.HYDRO_SIGMA
        if z >= 1.65:
            target = -200
        elif z >= 1.00:
        ...
        # Hysteresis: only adjust if delta > MIN_TRADE to avoid micro-churn
        if abs(target - position) < 10 and target != 0 and position != 0:
            return []
