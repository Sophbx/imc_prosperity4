...
class Trader:
    ...
    HG_FAIR = 10000.0
    HG_SIGMA = 32.0
    HG_KAPPA = 2.0           # target = -kappa * z * BASE_SIZE
    ...
    def _trade_hg_meanrev(self, state: TradingState, mem: Dict) -> List[Order]:
        ...
        if delta > CROSS_THRESH:
            ...
            cross_qty = min(30, delta, avail, self.HG_LIMIT - position)
            if cross_qty > 0:
                orders.append(Order(self.HG_SYM, ba, +cross_qty))
                position += cross_qty
                delta = target - position
        elif delta < -CROSS_THRESH:
            avail = int(depth.buy_orders[bb])
            cross_qty = min(30, -delta, avail, self.HG_LIMIT + position)
            if cross_qty > 0:
                orders.append(Order(self.HG_SYM, bb, -cross_qty))
                position -= cross_qty
                delta = target - position
        # Passive quote for the rest
        if delta > 0:
