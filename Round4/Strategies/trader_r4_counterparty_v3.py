"""
Trader v4 — merge of best-in-class strategies per product.

Round 4 counterparty candidate v1:
  - Starts from Round 3 v4.
  - Adds a narrow Mark 67 flow overlay for VELVETFRUIT_EXTRACT. In R4 tape,
    Mark 67 is the clearest directional participant: buyer-only, and VE tends
    to drift upward over the next 10-100 ticks after his buys.
  - v3 adds a conservative VEV Mark01/Mark14 shadow: passive long-only bids
    on VEV_5200/5300/5400/5500, where Mark 22 repeatedly pays the spread by
...
Components:
  ...
  - VEV_4500: specialized ITM MM (min_quote_spread=14, effectively no-op safety).
    Backtest: $0 / 3 days.
  - All other vouchers (5000-6500): SKIPPED.

Why we skip vouchers 5000-5500: the BS+momentum-reversal logic on these
strikes net +$276 / 3 days at best (with velvet_weight=0). Per-strike: -32,
-26, -22, +107, +494, -245. Not 100% confidently positive — choosing to
not trade them at all rather than risk variance.

Why skip 6000/6500: dead — mid pinned at 0.5 throughout, no edge.

Expected combined PnL: ~$102K / 3 days.
"""
...
class Trader:
    ...
    HYDRO_ALPHA = 0.03
    EXTRACT_ALPHA = 0.05
    VE_MARK67_DECAY_TS = 12_000.0
    VE_MARK67_BIAS_PER_LOT = 0.08
    VE_MARK67_MAX_BIAS = 2.0
    VEV_SHADOW_CAPS = {
        "VEV_5200": 40,
        "VEV_5300": 80,
        "VEV_5400": 80,
    ...
    def update_counterparty_flow(self, state: TradingState, mem: Dict) -> None:
        flow = mem.setdefault("flow", {})
        now = int(state.timestamp)
        last_ts = int(flow.get("last_ts", now))
        dt = max(0, now - last_ts)

        score = float(flow.get("ve_mark67", 0.0))
        if dt > 0:
            score *= math.exp(-dt / self.VE_MARK67_DECAY_TS)
        ...
        flow["ve_mark67"] = score
        flow["last_ts"] = now

    def mark67_ve_bias(self, mem: Dict) -> float:
        score = max(0.0, float(mem.get("flow", {}).get("ve_mark67", 0.0)))
        return min(self.VE_MARK67_MAX_BIAS, score * self.VE_MARK67_BIAS_PER_LOT)
    ...
    def trade_vev_shadow_bids(self, state: TradingState, product: str) -> List[Order]:
        depth = state.order_depths.get(product)
        if depth is None:
            return []
        ...
        position = int(state.position.get(product, 0))
        limit = self.POSITION_LIMITS[product]
        cap = int(self.VEV_SHADOW_CAPS.get(product, 0))
        size = int(self.VEV_SHADOW_SIZES.get(product, 0))
        qty = min(size, max(0, cap - position), max(0, limit - position))
        if qty <= 0:
            return []

        # Match Mark 01 / Mark 14: improve the bid by one tick, but never cross.
        bid_px = min(bb + 1, ba - 1)
        return [Order(product, int(bid_px), int(qty))]

    def run(self, state: TradingState):
        mem = self.load_memory(state.traderData)
        self.update_counterparty_flow(state, mem)
        ...
        # v4: only trade VEV_4000 and VEV_4500 (the specialized ITM strategy).
        # Skip VEV_5000-5500 (BS-momentum strategy net positive but tiny)
        # and VEV_6000/6500 (dead — mid pinned at 0.5).
        for product, strike in self.VOUCHER_STRIKES.items():
            if product not in self.ITM_VOUCHER_PRODUCTS:
                continue
        ...
        for vev_product in self.VEV_SHADOW_CAPS:
            shadow_orders = self.trade_vev_shadow_bids(state, vev_product)
            if shadow_orders:
                result[vev_product] = shadow_orders
