...
ASH_CONFIG = {
    ...
    # new: two-regime flattening only
    "flatten_aggressive_inventory": 40,
}
...
class ProductTrader:
    ...
    def _flatten_inventory(self, fv: float) -> None:
        """
        Two-regime flattening:

        Regime 1: soft_inventory < |pos| <= flatten_aggressive_inventory
            Flatten passively near fair.

        Regime 2: |pos| > flatten_aggressive_inventory
            Flatten more aggressively, one tick through fair.
        """
        soft = ASH_CONFIG["soft_inventory"]
        aggressive = ASH_CONFIG["flatten_aggressive_inventory"]
        ...
        if pos > 0:
            if abs_pos <= aggressive:
                # moderate long: passive flatten
                flatten_px = int(math.ceil(fv))
                flatten_qty = pos - soft
                self.ask(flatten_px, flatten_qty)
            else:
                # large long: more aggressive flatten
            ...
            if abs_pos <= aggressive:
                # moderate short: passive flatten
                flatten_px = int(math.floor(fv))
                flatten_qty = abs(pos) - soft
                self.bid(flatten_px, flatten_qty)
            else:
                # large short: more aggressive flatten
                flatten_px = int(math.ceil(fv))
                flatten_qty = abs(pos) - soft
                self.bid(flatten_px, flatten_qty)
...
class Round1AshTrader(ProductTrader):
    ...
    def get_orders(self) -> Dict[str, List[Order]]:
        ...
        # only changed section: flattening
...
class Trader:
    ...
    def bid(self) -> int:
        # Round 2 Market Access Fee (MAF) — paid only if we're in the top 50%
        # of all submitted bids. Winners get +25% order book flow.
        # 1000 is a middle-of-road pick: beats teams that default to 0,
        # low enough not to overpay if most teams bid conservatively.
        return 1000
