...
ASH_CONFIG = {
    ...
    # new: continuous inventory skew
    "inv_skew_price_ticks": 2.0,   # max price skew in ticks near position limit
    "inv_skew_size_frac": 0.75,    # max size reduction on same-side quoting
}
...
class ProductTrader:
    ...
    def _flatten_inventory(self, fv: float) -> None:
        soft = ASH_CONFIG["soft_inventory"]
    ...
    def _inventory_skew_price(self) -> int:
        """
        Positive return => shift both quotes downward (good when long).
        Negative return => shift both quotes upward (good when short).
        """
        frac = self.initial_position / max(self.position_limit, 1)
        raw = ASH_CONFIG["inv_skew_price_ticks"] * frac
        return int(round(raw))

    def _inventory_skew_sizes(self, base_size: int) -> Tuple[int, int]:
        """
        If long:
            shrink bid size, enlarge ask size mildly.
        If short:
            enlarge bid size mildly, shrink ask size.
        """
        frac = self.initial_position / max(self.position_limit, 1)
        shrink = ASH_CONFIG["inv_skew_size_frac"] * abs(frac)

        if frac > 0:
            bid_mult = max(0.20, 1.0 - shrink)
            ask_mult = min(1.50, 1.0 + 0.50 * shrink)
        elif frac < 0:
            bid_mult = min(1.50, 1.0 + 0.50 * shrink)
            ask_mult = max(0.20, 1.0 - shrink)
        else:
            bid_mult = 1.0
            ask_mult = 1.0
...
class Round1AshTrader(ProductTrader):
    ...
    def get_orders(self) -> Dict[str, List[Order]]:
        ...
        # new: continuous inventory skew in quote price
        inv_px_skew = self._inventory_skew_price()
        bid_price -= inv_px_skew
        ask_price -= inv_px_skew
        ...
        # new: continuous inventory skew in quote size
        base_size = ASH_CONFIG["base_quote_size"]
        bid_size, ask_size = self._inventory_skew_sizes(base_size)
